...
- ~~panel-bridge.sh git pull races DNS at boot~~ — fixed 2026-04-27, refined 2026-04-28. Even with `After=network-online.target` + `Wants=network-online.target` in the unit, NetworkManager-managed Pis can complete association before DNS is actually usable, so the first `git pull` attempt would fail with `Could not resolve hostname github.com` and the bridge would start with stale code (silently — the script already tolerates pull failure). The original fix retried the pull 5× with 5 s gaps; **that was sometimes still too short** — the user observed it failing on cold boot but a manual `git pull` working a few seconds later. Refined fix replaces blind retry with `getent hosts github.com` polling: wait up to 60 s for DNS resolvability before attempting the pull, then do one shot. If DNS never becomes resolvable (production deploys with WiFi disabled), skip the pull cleanly and start the bridge with what's on disk.
...
- ~~Forwarded entity payloads >1024 bytes silently dropped~~ — fixed 2026-04-27. esp-mqtt's default input buffer is 1024 bytes; anything larger is delivered as multiple `MQTT_EVENT_DATA` callbacks (one per buffer-full) with `current_data_offset` / `total_data_len` set. `panel_app.c`'s `forward_entity_state` / `forward_roster` / `forward_panel_set` helpers all treat each callback as a complete message and validate that the payload starts with `{` and ends with `}` — so the first chunk fails the trailing-`}` check, the second fails the leading-`{` check, both get dropped. Manifested as the PetLibro feeding-schedule binary_sensor (4 plans of nested attrs ≈ 1.3 KB) and the manual_feed_quantity select (48 unicode-fraction options) never reaching the UI while smaller sensors arrived fine. Fixed by setting `mqtt_cfg.buffer.size = 4096` in [`panel_net.c`](../platform/firmware/components/panel_platform/panel_net.c). Buffer later raised to 8 KB (2026-04-28) for headroom on burst republishes triggered by `cmd/resync`. Robust long-term fix would also handle multi-callback fragmentation in the forwarders, but bumping the buffer covers V1 payloads with headroom.
- ~~Boot-time data loss: kiosk starts blank because retained MQTT messages are dropped at the UART before the Pi is ready~~ — fixed 2026-04-28. The C6 has no separate power; it boots from the Pi's 5V rail. But the C6 finishes booting and MQTT-subscribing in ~3 s, well before the Pi finishes Linux boot and starts `panel-bridge`. The broker delivers all retained `state/entity/#` messages at subscription time, the C6 dutifully forwards them via UART, and **the Pi's serial driver isn't initialized yet so the bytes go to a UART with no reader — dropped at the line, no buffer involved**. Once the Pi catches up, the C6's MQTT subscription is still valid but retained messages don't get re-delivered (broker only sends them once on subscribe). Until HA next publishes a state change, the kiosk learns nothing — manifested as "blank UI for 5+ min after every Pi reboot, then a single state edit in HA fills exactly one entity in the kiosk and leaves the rest empty." Fix: a `cmd/resync` topic the bridge fires every time its UART link first comes up. Bridge sends `{"type":"panel_cmd","name":"resync"}` over UART; new C6 firmware handler ([`on_uart_line` panel_cmd branch](../panels/feeding_control/firmware/main/panel_app.c)) publishes that to MQTT; integration handler ([`_handle_resync` in forwarder.py](../custom_components/thread_panel/forwarder.py)) republishes the roster, every entity snapshot, and `ha_availability=online`. Same trigger covers cold boot, Pi reboot, bridge restart, UART cable glitch — anywhere the link comes back up. Required: reflashing C6 firmware, reloading the HA integration, restarting `panel-bridge`.
- ~~Boot-noise garbles the bridge's first UART write~~ — fixed 2026-04-28. Companion to the boot-time data loss bug. The Pi's UART line emits ~150 bytes of noise (likely BREAK signal or null-byte runs from the kernel driver coming up) during early boot. The C6's `rx_task` accumulates those bytes into its line buffer because nothing terminates them with `\n`. When the bridge later writes its first valid line — the `cmd/resync` payload terminated with `\n` — the C6's trailing-newline dispatch fires on the *concatenated* (boot-noise + resync) buffer and `strstr` against the garbled prefix can't match. Resync silently dropped, kiosk stays blank. Fix: prepend `\n` to every outgoing message in `panel_bridge/uart_link.py::send`. The leading newline flushes any accumulated junk first (dispatched as a no-op since the C6 ignores empty lines) before our content lands in a clean buffer. Cheap (one byte per message), bombproof.
