...
- ~~`install-pi.sh`'s MQTT-credentials prompt fails silently when the script is run via `curl | bash` — stdin is the script body (already consumed), so every `read` call returns empty and the validation loop spins forever printing "passwords don't match" / "username can't be empty".~~ ✅ DONE 2026-05-03 (bundled with [A.1.b](phase2_groupA_multipanel.md#a1b--per-pi-panel-identity--selective-artifact-download)). All three MQTT prompts (`mqtt_user`, `mqtt_pass`, `mqtt_pass_confirm`) now use `</dev/tty`, same fix pattern as the new panel_id prompt added in A.1.b.
